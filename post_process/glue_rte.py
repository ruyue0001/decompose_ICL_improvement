        ...
        with jsonlines.open(with_ICL_path) as f_w_icl:
            ...
            for wo_icl_line, w_icl_line in zip(f_wo_icl, f_w_icl):
                ...
                if model in ["chatgpt", "gpt3"]:
                    ...
                    if 'entail' in wo_icl_output and "true" not in wo_icl_output and "false" not in wo_icl_output and "neutral" not in wo_icl_output:
                        wo_icl_in_space.add(cnt)
                    ...
                    if 'entail' in w_icl_output and "true" not in w_icl_output and "false" not in w_icl_output and "neutral" not in w_icl_output:
                        w_icl_in_space.add(cnt)
            ...
            for wo_icl_line, w_icl_line in zip(f_wo_icl, f_w_icl):
                ...
                if cnt in new_ISIF_ISOOF:
                    if model in ["chatgpt", "gpt3"]:
                        ...
                        # print("=========\n", wo_icl_output, "\n---\n", w_icl_output, is_llama2_entailment(w_icl_output),"\n---\n", label)
                        if (is_llama2_entailment(w_icl_output) and label == "entailment") or (is_llama2_non_entailment(w_icl_output) and label == "non-entailment"):
                            cnt_org_isoof_icl_right += 1
    ...
    with jsonlines.open(file_name) as f:
        for line in f.iter():
            ...
            if model in ["chatgpt", "gpt3"]:
                ...
                if exp in ["acc2", "flip", "retrieval", "acc3", "num_k", "reverse_rand", "reverse_retrieval"]:
                    output = output.split('\n')[0]
