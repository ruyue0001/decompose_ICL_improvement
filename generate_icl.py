...
def main():
    ...
    if args.benchmark_name == 'huggingface':
        ...
        subtask_dataset = [{sentence1_key:e[sentence1_key], label_key:e[label_key]} for e in dataset]
        temp = pd.DataFrame(subtask_dataset)
        # test size 87
        pos = temp[temp[label_key]==1]
        neg = temp[temp[label_key]==0]
        pos_test_size = math.trunc(len(pos) * 0.2)
        neg_test_size = 87 - pos_test_size
        pos_test = pos.sample(pos_test_size)
        neg_test = neg.sample(neg_test_size)
        pos_train=pos.drop(pos_test.index)
        neg_train=neg.drop(neg_test.index)
        
        train_dataset = pd.concat([pos_train, neg_train]).to_dict('records')
        test_dataset = pd.concat([pos_test, neg_test]).to_dict('records')
    ...
    if args.task_name in ['conll2003','wnut_17','bc2gm_corpus']: #sequence tagging tasks
        test_label_count = len(set([i for _ in test_dataset[label_key] for i in _]))
        train_label_count = len(set([i for _ in train_dataset[label_key] for i in _]))
        ...
        print(f'Num class : {train_label_count}')

        train_dataset = [sample for sample in train_dataset]
    
    elif args.task_name in ['rest14', 'laptop14', 'rest15', 'rest16']: # ABSA
        train_dataset = [sample for sample in train_dataset]
    elif args.benchmark_name == 'ethos':
        pass
        ...
        train_dataset_per_class_list = [list(filter(lambda sample: sample[label_key] == class_index, train_dataset)) for
                                        class_index in range(num_class)]
        ...
        print('TEST ========================')
        # split train set for balanced selection
        test_dataset_per_class_list = [list(filter(lambda sample: sample[label_key] == class_index, test_dataset)) for
                                       class_index in range(num_class)]
    ...
    for i in tqdm(range(args.n_samples), desc="Sampling training set"):
        ...
        # for config
        if args.task_name not in ['conll2003','wnut_17','bc2gm_corpus', 'rest14', 'laptop14', 'rest15', 'rest16']:
